...
public class MessageService {
    ...
    public Message sendMessage(Long senderId, String senderType,
        ...
        // 验证发送者和接收者的权限，系统消息（senderId=0）跳过权限检查
        if (senderId != 0L && !hasPermissionToChat(senderId, senderType, receiverId, receiverType, courseId)) {
    ...
    private String getUserName(Long userId, String userType) {
        // 系统通知的特殊处理
        if (userId == 0L && "ADMIN".equalsIgnoreCase(userType)) {
            return "系统通知";
        }

        if ("TEACHER".equalsIgnoreCase(userType)) {
            return teacherRepository.findByUserId(userId).map(Teacher::getRealName).orElse(null);
        } else if ("STUDENT".equalsIgnoreCase(userType)) {
            return studentRepository.findByUserId(userId).map(Student::getRealName).orElse(null);
        } else if ("ADMIN".equalsIgnoreCase(userType)) {
            return userRepository.findById(userId).map(user -> user.getRealName() != null ? user.getRealName() : user.getUsername()).orElse(null);
        }
        return null;
    }
